...
The host renders at a device scale — each logical pixel occupies `scale × scale`
physical pixels — and floors cart coordinates in device space, not logical space.
A sprite moving diagonally at a sub-pixel speed therefore advances smoothly frame
to frame: carts store an exact `f32` position and pass it directly to draw calls;
no cart-side motion helper is needed.
