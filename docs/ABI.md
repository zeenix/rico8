...
`rico8_fps` reports the cart's logical frame rate: `60` runs `update` and
`draw` 60 times per second; any other value, or a missing export, means 30.
Because it is optional and defaults to 30, carts built before it existed
still load and run unchanged — no cart-format version bump.
