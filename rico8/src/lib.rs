...
//! palette has 16 fixed colors, `update`/`draw` run at 30 fps (or 60,
...
///
/// The default is [`FrameRate::Fps30`]; override [`Rico8Game::frame_rate`]
/// to return [`FrameRate::Fps60`] for a 60 fps game, where both `update`
/// and `draw` are called twice as often.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum FrameRate {
    /// 30 frames per second (the default).
    Fps30,
...
pub trait Rico8Game {
    ...
    fn draw(&self, gfx: &mut Graphics);
    /// The logical frame rate. Override to return [`FrameRate::Fps60`] to
    /// run `update` and `draw` at 60 fps instead of the default 30.
